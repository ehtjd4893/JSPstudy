...
<body>
	<%
		...
		// 1. 파일이 업로드될 디렉터리명을 변수로 지정한다.
		String directory = "archive";
		// 2. 파일이 업로드 될 디렉터리의 실제 경로를 변수로 지정한다.
		String realPath = request.getServletContext().getRealPath(directory);
	
		// 3. 디렉토리가 생성되지 않았다면 새로 생성한다.
		File dir = new File(realPath);
		if(!dir.exists()){	// 디렉터리가 없다면
			dir.mkdirs();	// 디렉터리를 만든다.
		}
		
		// 4. 기존 request를 사용하지 않고, MultipartRequest 클래스 타입의 객체를 생성
		//	  객체를 만들면 업로드가 된다.
		MultipartRequest multipart = new MultipartRequest(
				request, 			// 기존 request를 전달
				realPath,			// 업로드 될 경로 지정
				1024 * 1024 * 10,	// 10MB(최대 업로드 크기 1024B(1KB), 1024KB(1MB)
				"UTF-8", 			// 인코딩
				new DefaultFileRenamePolicy()	// 기본파일명 변경정책 (원래 파일명에 숫자 추가)	
				);
	...
	<ul>
		...
		<%
			...
			// length() 메소드는 바이트를 반환하므로 1024로 나누어 KB로 저장한다.
