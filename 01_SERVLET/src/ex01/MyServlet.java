 ...
 * - 웹 브라우저 url은 http://localhost:포트번호/컨텍스트패스/url매핑 방식으로 구성됨.
 */
@WebServlet("/MyServlet")	// url 매핑이다.
public class MyServlet extends HttpServlet {	// httpServlet 클래스를 상속 받으면 서블릿이다.
	private static final long serialVersionUID = 1L;	// 직렬화를 위한 id값이다. 여기서는 관심 없다.
	 ...
	 *  	2) <form method="get">
	 *  	3) $.ajax({type:'get'})
	 *  	- service() 메소드를 사용하지 않은 경우 자동으로 호출된다. 
	 *  
	 */
	 ...
	 *  - 실제로는 아무 처리도 하지 않고, 모든 처리를 doGet() 메소드에 위임.
	 */
